...
export function StayCard({ stay }: StayCardProps) {
  return (
    <article className="group overflow-hidden rounded-none border border-gray-200 bg-white transition-all duration-300 hover:-translate-y-1 hover:border-[#1a1a1a]">
      <Link href={`/stays/${stay.slug}`} className="block">
        ...
        <div className="p-5">
          ...
          <h3 className="mt-2 text-[17px] font-bold text-[#1a1a1a]">{stay.name}</h3>
          ...
          <div className="mt-4 flex items-center justify-between border-t border-gray-100 pt-3">
            ...
            <span className="text-[14px] font-semibold text-[#1a1a1a]">{stay.price}~</span>
          </div>
        </div>
      </Link>
    </article>
