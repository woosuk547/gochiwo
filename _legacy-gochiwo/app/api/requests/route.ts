...
import { sendRequestConfirmation } from '@/lib/mailer'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { title, description, email, address, latitude, longitude, imageUrl } = body
    ...
    // 이메일 주소가 있으면 접수 확인 메일 발송
    if (email) {
      sendRequestConfirmation(email, newRequest.id).catch(() => {
        // 메일 발송 실패는 요청 접수에 영향 없음
      })
    }
